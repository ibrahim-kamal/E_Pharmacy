...
?>
			<div class="container-fluid divArticle">
				<div>
					<div class="title">
						<a href="<?php echo $data['url'][$i]?>"><h3><?php echo $data['article_title'][$i]?></h3></a>
					</div>
					<div class="info">
						<div class="img">
						...
						<div class="body">	
							...
							<div class="desc">
								<h4><?php echo $data['article_subject'][$i]?></h4>
							</div>
						</div>
						<div class="change">
							<span><i title="remove article" data-id="<?php echo $data['articleid'][$i];?>" class="remove glyphicon glyphicon-remove"></i></span>
							<?php if (isset($edit)){ ?>
								<span><i title="edit article" data-id="<?php echo $data['articleid'][$i];?>" class="edit glyphicon glyphicon-edit"></i></span>
							<?php }?>
						</div>
					</div>
				</div>
...
<script>
	...
	var link = "<?php echo base_url().'Articlecont/removeArticle'?>" ;
	 $.ajax({
	        url: link,
...
<?php if (isset($edit)){ ?>
	<script>
		$('body').on('click','.edit',function(e){
			var data_id = $(this).attr('data-id');
			location.href ="<?php echo base_url()."Doctorcont/editArticle/"?>"+data_id;
		});
	</script>
<?php }?>